...
- (void)contentSetup {
    self.titles = @[@"椿と楓",
                    @"地図",
                    @"危険地域"];
    self.texts = @[@"赤い服の椿は情報を収集し、\n橙色の服の楓は詳細を伝えに行きます",
                   @"椿があなたの周辺の地図を出してくれて、\n近くの名所やホテルなどあなたに伝授します。",
                   @"楓が海外の危険な地域を教えてくれて、\nあなたに注意を促すよう教えてくれます。"];
...
- (__kindof UIView *)staticContentView {
    ...
    staticView.startButtonTapBlock = ^void() {
        MISForgetItemsTableView *animalsController = [self.storyboard instantiateViewControllerWithIdentifier:@"MISForgetItemsTableView"];
        [self presentViewController:animalsController animated:YES completion:nil];
    };
    [staticView setupButtonText:@"忘れ物チェックを始めよう"];
